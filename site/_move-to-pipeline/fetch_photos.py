...
Output: photos/mk/<MkId>.<ext> + photos/mk/index.json, NEXT TO THIS SCRIPT
        ({"<MkId>": "<filename>"} — the page shows photos ONLY for ids in
        that manifest; nobody ever gets a guessed URL or a broken image).
        Since 2026-09-06 the site keeps no photos of its own: this folder is
        pipeline territory, and the pipeline publishes the manifest and the
        images to Cloudflare (see HANDOFF-cloudflare-data.md in site\). The
        page reads <relay>/data/mkphotos and <relay>/photos/mk/<file>.
...
OUT = HERE / "photos" / "mk"
# the relay address is read from the site's config.js; the site folder is a
# sibling of the pipeline (…\NewsWebsite\site\) — or a parent, while this
# script still sits inside site\_move-to-pipeline\
CONFIG_CANDIDATES = [HERE / "config.js", HERE.parent / "config.js",
                     HERE.parent / "site" / "config.js", HERE.parent.parent / "site" / "config.js"]
...
def main():
    ...
    for mk_id, name in people:
        ...
        try:
            ...
            img_url = (hdr or {}).get("MkImage") or None
        except Exception:
            pass
        data = None
        if img_url:
    ...
    print()
    print("photos/mk/index.json is up to date. The site shows them once the pipeline")
    print("publishes this folder to Cloudflare (HANDOFF-cloudflare-data.md in site\\).")
    print("after an election: run me again — new members get added.")
