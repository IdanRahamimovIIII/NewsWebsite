...
/* ---------- the ministry's own figure, read from its own report ----------
   ...
   tools/parse_report.py builds site/data/paid/<section>.json:
   ...
   A missing file is not an error; most sections have none yet. */
/* data/paid/index.json lists the sections we have a file for. Without it the
   page cannot tell "no file for this ministry yet" (normal — most sections)
   from "site/data never got deployed" (a bug that otherwise shows up as
   dashes everywhere and no error anywhere). state.paidData records which. */
const reportedCache = {};
let manifestPromise = null;
function loadPaidManifest() {
  if (!manifestPromise) {
    manifestPromise = fetch("data/paid/index.json")
      .then(r => r.ok ? r.json() : null)
      .catch(() => null)
      .then(m => {
        state.paidData = m && Array.isArray(m.sections) ? m.sections : null;
        if (!state.paidData)
          debug("paid overlay: data/paid/index.json not found — is site/data deployed?");
...
function loadReported(section) {
  if (!(section in reportedCache)) {
    reportedCache[section] = fetch("data/paid/" + section + ".json")
      .then(r => r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status)))
      .catch(e => { debug("paid overlay " + section + ": " + e.message); return null; });
  }
