...
REM
REM  ONE output:
REM    out\contracts-public.db     (here in contractors\) - the file
REM                                upload-to-d1.bat sends to Cloudflare,
REM                                INCLUDING the page's precomputed ctr_*
REM                                tables (build_contractors.py runs at the
REM                                end, so a rebuild never loses them).
REM
...
if %errorlevel%==0 (
  python contractors\build_database.py
  goto :done
)

where py >nul 2>nul
if %errorlevel%==0 (
  py contractors\build_database.py
  goto :done
)
