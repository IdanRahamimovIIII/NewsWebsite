...
(the legislative freezes). Zero-install: stdlib only.
  ...
  python fetch_avgwage.py --publish  ...then publish KV pub:avgwage
                                     -> served as /data/avgwage

No raw archive: latest copy only (Mercy — raw-forever is a contracts-dataset
rule, not a general one). Publishing uses shared\\cf_kv.py (env
CF_API_TOKEN/CF_ACCOUNT_ID in CI, pipeline\\d1-config.json on Mercy's machine).
"""
import json
import re
import sys
...
def publish(data: dict) -> int:
    ...
    cf_kv.bulk_put(cred, ns, [("pub:avgwage", value)])
    bad = cf_kv.verify(cred, ns, [("pub:avgwage", value)])
    if bad:
        print("FAIL publish read-back:", "; ".join(bad))
        return 1
    print("OK   published pub:avgwage (%d bytes) — read back and matched" % len(value))
