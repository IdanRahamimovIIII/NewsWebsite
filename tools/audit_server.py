...
locally." So this serves site/ like serve.bat does, PLUS a few read-only
/audit endpoints answered from the FULL contracts.db — the one with
provenance per field and the raw register rows embedded. Nothing here is
deployed anywhere; closing the window ends it.
...
usage:
  audit_server.py [--db contracts.db] [--site site] [--port 8081]
"""
...
def open_db(path):
    if not os.path.exists(path):
        sys.exit("no database at %s — build it first:\n"
                 "  python tools/build_dataset.py … && python tools/build_sqlite.py …\n"
                 "or copy contracts.db into the project folder." % path)
...
def main():
    ...
    ap.add_argument("--db", default="contracts.db")
    ...
    if not os.path.isdir(a.site):
        sys.exit("no %s directory here — run from the project root" % a.site)
    os.chdir(a.site)
    ...
    print("open http://localhost:%d/tools/compare.html" % a.port)
